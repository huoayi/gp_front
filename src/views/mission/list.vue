...
async function getData(params: { [key: string]: any } & IPagination, setSpinning: boolean = true) {
  try {
    setSpinning && (spinning.value = true);
    autoRefreshLoading.value = true;
    // console.log('mission params', toRaw(params));
    state.params = params;
    const parameters: IMissionListParams & Record<string, any> = { ...params };
    if (params.start && params.end) {
      Object.assign(parameters, { started_at: params.start, finished_at: params.end });
      delete parameters.start, delete parameters.end;
    }
    const keys = Object.keys(params);
    // 状态
    parameters.front_state = tabList.find((i) => i.key === state.activeTabKey)?.values || [];

    const sortMapping: Record<string, string> = {
      startedAt: 'des_start_at',
      expiredAt: 'des_end_at',
      finishedAt: 'des_stop_at',
    };
    const sortKeys = keys.filter((k) => Object.keys(sortMapping).includes(k));
    sortKeys.forEach((k) => {
      let key = sortMapping[k];
      if (params[k]) Object.assign(parameters, { [key]: params[k] === 'desc' ? true : false });
      delete parameters[k];
    });
    console.log('mission parameters', parameters);

    const result = await getMissionList(parameters);
    ...
    let subMissionIds: Record<string, number> = {};
    let resultList = (list || []).map((item: any, index: number) => {
      let {
        id,
        mission_type: appType,
        mission_way: useWay,
        state,
        ...
        total_consume,
        urls,
        type, // sd_time | ssh_time 等
        temp_hmac_key: hk,
        temp_hmac_secret: hs,
        username: account,
        password,
        // 1. 计费方式 edges?.mission_orders，失败没有这个 mission_orders（因为续费操作所以可能会存在多个订单）
        // 2. 自动续费 edges?.renewal_agreements
        edges,
        new_device: hasRemainFree, // 表示在新机器上
      } = item;

      // 应用名称、版本、GPU 版本
      let appName = '',
        appVersion = '/',
        gpuVersionText = getGpuVersionText(gpuVersion);
      const appItem = subOptions.app.find((i) => i.value === appType);
      if (appItem) {
        const { label, children } = appItem;
        appName = label;
        const cItem = (children || []).find((i) => type.includes(i.type));
        cItem && (appVersion = cItem.text);
      }

      let isKeyPair = type === 'key_pair';
      if (isKeyPair) subMissionIds[id] = index;
      let { isTime, label: chargeWay = '' } = subOptions.charge.find((i) => i.value === billingType) || {};
      isTime = isTime === undefined ? true : isTime; // 没有默认是计时
      let isPlanTime = isTime && billingType !== 'time';
      // 计时情况下状态是 supplying 或 计次情况下状态是 doing 时，是已启动
      let statusValue =
        (isTime && state === 'supplying') || (!isTime && state === 'doing')
          ? 'doing'
          : state === 'closing'
          ? state
          : ['canceled', 'failed', 'succeed'].includes(state)
          ? 'ended'
          : isTime && state === 'doing'
          ? 'starting'
          : 'waiting';

      let isRunning = statusValue === 'doing',
        isWaiting = statusValue === 'waiting',
        isStarting = statusValue === 'starting',
        isEnded = statusValue === 'ended',
        isCloseMachine = false;

      // data-item
      let { id: renewalId, next_pay_time, sub_status } = (edges?.renewal_agreements || [])[0] || {},
        isAutoRenewal = sub_status === 'subscribing'; // 是否订阅中
      let startedAt = getTimeText(started_at),
        expiredAt = getTimeText(expired_at),
        finishedAt = getTimeText(finished_at),
        renewalAt = getTimeText(next_pay_time),
        createdAt = getTimeText(created_at),
        freeAt = getTimeText(free_at),
        remainFreeTime = '/'; // 剩余释放时间

      let openWay = '',
        sshOpenWay = '',
        showFileManagement = !!appItem?.hasFileManagement && isTime, // 展示文件管理
        fileManagementData: { useIntroLink?: string; fileLink?: string; logLink?: string } | null = showFileManagement
          ? { useIntroLink: 'https://cephalon.feishu.cn/docx/NgxWdvvi6oYa8dxrCCrcGdoAnhe?from=from_copylink' }
          : null,
        validateWayData = ['JP'].includes(appType)
          ? { password: '' }
          : !isKeyPair
          ? { account: '', password: '' }
          : null;
      // card 到期时间、续费时间、非包时长不展示
      let secondTimeName = '',
        secondTime = '';
      ...
      if (isRunning) {
        ...
        if (Array.isArray(tempUrls)) {
          ...
          if (tempUrls[1]) {
            ...
            fileManagementData.fileLink = tempUrls[1];
            fileManagementData.logLink = `${tempUrls[1]}/files/run.log`;
          }
        }
        // 验证：todo：sd 随机，其他暂由前端
        validateWayData = ['JP'].includes(appType)
          ? { password: 'cephalon' }
          : ['SSH'].includes(appType)
          ? { account: 'admin', password: 'screencast' }
          : ['ASCEND'].includes(appType)
          ? { account: 'root', password: 'bE2c09uznE' }
          : /^SD|COMFYUI|FOOOCUS|LORA/.test(appType)
          ? { account, password }
          : { account: 'admin', password: 'cephalon' };
        if (isKeyPair && hk && hs) {
          validateWayData = { 'Hmac-Key': hk, Hmac: hs } as any;
        }
        // 终端地址：openWay
        if (['SSH', 'Ascend'].includes(appName) && openWay) {
          let temp = openWay.replace(':', ' -p ');
          sshOpenWay = `ssh ${validateWayData?.account}@${temp}`;
        }
        // 包时长计费
        if (isPlanTime) {
          secondTimeName = isAutoRenewal ? '续费时间' : '到期时间';
          secondTime = isAutoRenewal ? renewalAt : expiredAt;
        }
      } else if (!(isWaiting || isStarting)) {
        // 已关机
        secondTimeName = '停用时间';
        secondTime = finishedAt;
        // 成功情况下，计算释放时间

        if (state === 'succeed' && freeAt !== '/' && hasRemainFree) {
          let diff = dayjs(free_at).diff(dayjs());
          if (diff > 0) {
            let temp = dayjs.duration(diff),
              day = temp.days(),
              hour = temp.hours();
            remainFreeTime = ` ${day} 天 ${hour} 小时`;
            isEnded = false;
            isCloseMachine = true;
          }
        }
      ...
      return {
        ...
        missionType: type,
        appType,
        appName, // appSubOptions.value 调接口需要时间
        appVersion,
        useWay,
        useWayText: subOptions.useWay.find((i) => i.value === useWay)?.label || '',
        statusValue,
        statusText: keyText[statusValue],
        gpuVersion,
        gpuVersionText,
        chargeWay,
        isAutoRenewal,
        renewalId,
        billingType, // 自动续费和重新开机会用到
        startedAt,
        expiredAt,
        finishedAt,
        createdAt,
        remainFreeTime,
        hasRemainFree,
        consume: convertToCEP(total_consume),
        consumeRMB: transferCepToRMB(convertToCEP(total_consume)),
        subCount: 0,
        openWay,
        sshOpenWay,
        showFileManagement,
        ...
        secondTimeName,
        secondTime,
        buyDuration: (edges?.mission_orders || [])[0]?.buy_duration,
        apiFileLink: 'https://cephalon.feishu.cn/docx/XLrVd6I2Gox6huxZU9lcaguZn58?from=from_copylink', // 计次目前只有 SD
      };
    });
    // console.log(1, subMissionIds);
    let subIds = Object.keys(subMissionIds);
    if (subIds.length > 0) {
      const subList = await getSubMissionsList({ mission_ids: subIds });
      if (subList.code === 20000) {
        subList.data?.forEach((item: any) => {
          let { mission_id, sub_count, sum_sub_cep } = item;
          let index = subMissionIds[mission_id];
          resultList[index].consume = convertToCEP(sum_sub_cep);
          resultList[index].subCount = sub_count;
        });
      }
    }
    datasource.value = resultList;
  } catch (err) {
